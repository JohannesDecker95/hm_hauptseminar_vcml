...
class GSBackEnd(mp.Process):
    ...
    @torch.no_grad()
    def eval_rendering(self, gtimages, gtdepthdir, traj, timestamps):
        # Extract kf_idx from viewpoints or use a default
        kf_idx = list(self.viewpoints.keys()) if self.viewpoints else []
        
        eval_rendering(gtimages, gtdepthdir, traj, self.gaussians,self.save_dir, self.background,
            self.projection_matrix, self.K, kf_idx, iteration="after_opt", timestamps=timestamps)
