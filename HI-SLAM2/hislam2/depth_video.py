    ...
    def __item_setter(self, index, item):
        ...
        try:
            ...
            if "expanded size" in str(e):
                # Handle tensor size mismatch by reshaping if needed
                if hasattr(item[0], 'shape') and len(item[0].shape) > 0:
                    if item[0].numel() == 1:
                        self.tstamp[index] = item[0].item()
                    else:
                        # If multiple timestamps, take the first one
                        self.tstamp[index] = item[0][0] if item[0].shape[0] > 0 else item[0]
                else:
                    self.tstamp[index] = item[0]
            else:
                raise e
