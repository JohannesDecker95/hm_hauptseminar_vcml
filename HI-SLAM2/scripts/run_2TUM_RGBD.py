...
out = 'outputs/2TUM_RGBD'
os.makedirs(f'{out}/meshes', exist_ok=True)
seqs = sorted(glob('data/TUM_RGBD/rgbd_dataset_*')) # + sorted(glob('data/TUM_RGBD/off*'))
...
for seq in seqs:
    ...
    cmd += f'--config config/owndata_config.yaml --calib calib/tum_rgbd.txt --output {out}/{name} > {out}/{name}/log.txt'
    ...
    if not os.path.exists(f'{out}/{name}/eval_recon_{weight}.txt'):
        os.system(f'python scripts/eval_recon.py {out}/{name}/tsdf_mesh_{weight}_aligned.ply data/TUM_RGBD/gt_mesh_culled/{name}.ply --eval_3d --save {out}/{name}/eval_recon_{weight}.txt > /dev/null')
