...
out = 'outputs/2HM_SLAM_autonome_systeme'
...
for seq in seqs:
    ...
    cmd += f'--config config/owndata_config.yaml --calib calib/hm_slam.txt --output {out}/{name} > {out}/{name}/log.txt'
    ...
    if not os.path.exists(f'{out}/{name}/eval_recon_{weight}.txt'):
        os.system(f'python scripts/eval_recon.py {out}/{name}/tsdf_mesh_{weight}_aligned.ply data/HM_SLAM_autonome_systeme/gt_mesh_culled/{name}.ply --eval_3d --save {out}/{name}/eval_recon_{weight}.txt > /dev/null')
